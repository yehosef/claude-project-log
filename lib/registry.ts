 ...
 * All files under the install dir (import.meta.dir). Atomic writes via tmp+rename.
 */
...
/** Default ignore list: scratch dir (derived at runtime) plus system temp dirs. */
...
export function loadIgnore(): string[] {
  if (!existsSync(IGNORE_PATH)) {
    const defaults = defaultIgnore();
    atomicWrite(IGNORE_PATH, defaults);
    return defaults;
  }
  return readJson<string[]>(IGNORE_PATH, defaultIgnore());
}
...
export function loadConfig(): Config {
  ...
  if (!cfg.code_roots) cfg.code_roots = [];
  return cfg;
}
