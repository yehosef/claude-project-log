...
// Resolve claude and bun binaries at runtime — never hardcode paths
const CLAUDE_BIN = Bun.which("claude") ?? "claude";
const BUN_BIN = process.execPath; // the bun that is running us right now
  ...
  if (!existsSync(NOTION_JSON_PATH)) {
    throw new Error(
      `notion.json not found at ${NOTION_JSON_PATH}. ` +
        `Run the setup script: bun <plugin-root>/lib/setup.ts`
    );
  }
  ...
  if (existsSync(LOCK_PATH)) {
    ...
    if (lockData) {
      ...
      if (pidAlive && ageMs < 10 * 60 * 1000) {
        // Lock is valid — exit silently
        return false;
      }

      // Stale lock — reclaim
      log(`Reclaiming stale lock (pid=${lockData.pid}, age=${Math.round(ageMs / 1000)}s)`);
      unlinkSync(LOCK_PATH);
    }
  }

  // Create lock (O_CREAT|O_EXCL equivalent: write with wx flag)
  try {
...
function runClaudeSynth(digest: string): SynthResult | null {
  ...
  // Build PATH from the running process's PATH, augmenting with dirs of the
  // resolved bun and claude binaries so launchd (which inherits a minimal PATH)
  // can still find them. Inherit the full env so claude's keychain credential
  // lookup gets USER/LOGNAME/HOME — a minimal env yields "Not logged in".
  const extraDirs: string[] = [];
  const claudeDir = dirname(CLAUDE_BIN);
  const bunDir = dirname(BUN_BIN);
  if (claudeDir && claudeDir !== ".") extraDirs.push(claudeDir);
  if (bunDir && bunDir !== ".") extraDirs.push(bunDir);

  const basePath = process.env.PATH ?? "/usr/local/bin:/usr/bin:/bin";
  const augmentedPath = [...new Set([...extraDirs, ...basePath.split(":")])].join(":");
...
/** Guess area from path. Returns a generic default; users can override via --area. */
function guessArea(cwd: string): string {
  if (cwd.includes("/personal/")) return "Personal";
  if (cwd.includes("/work/")) return "Work";
  return "Personal";
}
